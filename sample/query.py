...
def get_int(prompt):
    ...
    Note:
    The function will repeatedly ask the user
...
def lst_of_items(lst1, lst2):
    """
    Print items from two given lists, typically
    representing warehouse inventories.
    Parameters:
...
def ask_for_max(name, total, item_name):
    ...
    This function is executed when user types in
    more amount than the 'total' parameter.
    """
    while True:
...
def ask_for_placing_order(name, total, item_name):
    ...
    Various messages are shown based on the
    available stock and desired quantity.
    """
    continue_loop = True
    while continue_loop:
...
def searching_for_item(name):
    ...
    This function makes use of two lists,
    warehouse1 and warehouse2, from data module
    """
    while True:
...
def main():
    ...
    Returns:
    None. The function interacts with the user via
